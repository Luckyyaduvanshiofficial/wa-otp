...
There are two shapes in this app, and which one you use follows the data source, not a
house style.

**PocketBase-backed forms** (auth, profile, password) await the SDK directly inside
`onSubmit` and surface failures with a sonner toast — see
...
Longer ones add a local saving flag around the same shape
(`src/features/settings/components/settings-view.tsx`).

**API-backed actions that need cache invalidation** use TanStack Query mutations in the
view component instead — see `src/features/keys/components/keys-view.tsx`:
...
A form is never wrapped in an outer React Query mutation just to submit it.
